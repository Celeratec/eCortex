# MeshCentral Server Deployment Guide

## DFW MSP Backup Remote Access System

This guide covers deploying MeshCentral as a self-hosted backup remote access solution for DFW MSP technicians.
...
MeshCentral serves as a **backup remote access system** when primary tools fail:

| Scenario | Primary Tool | Backup (MeshCentral) |
|----------|-------------|---------------------|
...
```
mesh.dfwmsp.com → <SERVER_PUBLIC_IP>
```
...
1. Open `https://mesh.dfwmsp.com` in browser
2. Accept the certificate (or wait for Let's Encrypt)
3. You should see the MeshCentral login page
...
```bash
# Using MeshCtrl
meshctrl --url wss://mesh.dfwmsp.com \
         --loginuser admin \
         --loginpass 'password' \
         AddAgentInviteCode \
